...
\U0001F9D4 \U0001F469 If you want to add your name to the existing room just write the key of the room and your name through the colon. 
Example: jwucjshl:Andrei Ternikov

\U0001F3C1 Only the creator of the room can start the distribution of the names.
To do so, send me the command /start and the key of the room through the colon. Example: /start:jwucjshl""")
...
Now players of the game should be added. Everybody should add themselves. 
Example:jwucjshl:Andrei Ternikov""")
    except:
        bot.reply_to(message, 'Something went wrong.. Try to send the command again.')


@bot.message_handler(func=lambda message: 'start' in message.text)
def start_room(message):
    conn, cur = connect_to_db()
    room = ':'.join(message.text.split(':')[1:])
    try:
        ...
        targets = list(range(users_count))
        if users_count % 2 == 0:
        ...
        for target_number, user in enumerate(users):
            bot.send_message(chat_id=user[0], text=f"""Secret Santa in the room {room} is started!
                
You are a Secret Santa of {users[targets[target_number]][1]} \U0001F381""")

        bot.reply_to(message, f"""You started the room {room}!""")
    except:
        bot.reply_to(message, f"""This room doesn't exist or you are not the creator of the room""")


@bot.message_handler(func=lambda message: ':' in message.text)
def add_user(message):
    conn, cur = connect_to_db()
    room = message.text.split(':')[0]
    ...
    try:
        ...
        conn.commit()
        bot.reply_to(message, f"""The player with the name {name} is successfully added to the room {room}!

Let's wait, when the creater starts the game and distributes the names. If it is you press the command /start and the key of the room. Example: start:jwucjshl""")
    except:
        bot.reply_to(message,
                     """Something went wrong. This room doesn't exist or the player is already in the room.""")
